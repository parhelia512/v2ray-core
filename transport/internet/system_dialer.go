...
func ApplySockopt(sockopt *SocketConfig, dest net.Destination, fd uintptr, ctx context.Context) {
	if err := applyOutboundSocketOptions(dest.Network.String(), dest.Address.String(), fd, sockopt, dest); err != nil {
		newError("failed to apply socket options").Base(err).WriteToLog(session.ExportIDToError(ctx))
	}
	if dest.Network == net.Network_UDP && sockopt.HasBindAddr() {
		if err := bindAddr(fd, sockopt.BindAddress, sockopt.BindPort); err != nil {
...
func (c *PacketConnWrapper) Write(p []byte) (int, error) {
	if c.Oob == nil {
		...
		n, _, err := c.Conn.WriteMsgUDP(p, c.Oob, c.Dest)
		return n, err
	}
}
...
func UseDefaultInterface(bind4 net.IP, bind6 net.IP, name string, index int) {
	defaultBind4 = bind4
	defaultBind6 = bind6
