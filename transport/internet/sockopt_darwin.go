...
func applyOutboundSocketOptions(network string, address string, fd uintptr, config *SocketConfig, dest net.Destination) error {
	...
	if config.BindInterfaceIndex != 0 {
		if err := bindInterface(fd, network, config.BindInterfaceIndex, dest); err != nil {
			return err
		}
	}
...
func applyInboundSocketOptions(network string, fd uintptr, config *SocketConfig) error {
	...
	if config.BindInterfaceIndex != 0 {
		if err := bindInterface(fd, network, config.BindInterfaceIndex, noDestination); err != nil {
			return err
		}
	}
...
var noDestination net.Destination

func bindAddr(fd uintptr, address []byte, port uint32) error {
	return nil
}

func bindInterface(fd uintptr, network string, interfaceIndex uint32, dest net.Destination) error {
	switch network {
	case "tcp4", "udp4":
