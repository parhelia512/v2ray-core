...
func (c RootConfig) BuildV5(ctx context.Context) (proto.Message, error) {
	...
	if c.TunConfig != nil {
		tunApp, err := loadHeterogeneousConfigFromRawJSON("service", "tun", c.TunConfig)
		if err != nil {
			return nil, newError("failed to parse TUN config").Base(err)
		}
		config.App = append(config.App, serial.ToTypedMessage(tunApp))
	}
